    ...
    <div class="messages-content">
      <el-tabs v-model="activeTab">
            ...
            >
              ...
              >
                <div class="message-header">
                  <div class="message-title">{{ msg.title || getMessageTitle(msg) }}</div>
        ...
        <el-tab-pane label="审核通知" name="audit">
          ...
          <div v-else-if="auditMessages.length === 0" class="empty-messages">
            <el-empty description="暂无审核通知" :image-size="100" />
          </div>
          <div v-else class="message-list">
            <div
              v-for="msg in auditMessages"
              :key="msg.noticeId"
              class="message-item-container"
            >
              <!-- 删除按钮区域（在消息卡片下方） -->
        ...
        <el-tab-pane label="库存预警" name="inventory">
          ...
          <div v-else-if="inventoryMessages.length === 0" class="empty-messages">
            <el-empty description="暂无库存预警" :image-size="100" />
          </div>
          <div v-else class="message-list">
            <div
              v-for="msg in inventoryMessages"
              :key="msg.noticeId"
              class="message-item-container"
            >
              <!-- 删除按钮区域（在消息卡片下方） -->
...
const auditMessages = computed(() => {
  return messageStore.auditMessages as SwipeMessageItem[]
})

// 库存预警
const inventoryMessages = computed(() => {
  return messageStore.inventoryMessages as SwipeMessageItem[]
})
...
const getMessageTitle = (msg: SwipeMessageItem): string => {
  if (msg.noticeType == 0) {
    ...
    return '评价通知'
  } else if (msg.content.includes('审核')) {
    return '审核通知'
  } else {
    return '消息通知'
  }
