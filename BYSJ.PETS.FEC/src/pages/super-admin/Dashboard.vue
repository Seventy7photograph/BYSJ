    ...
    <el-card shadow="hover" class="welcome-card">
      <template #header>
        <div class="card-header">
          <span>欢迎回来，{{ currentUser?.username || '超级管理员' }}！</span>
        </div>
      </template>
      ...
      <div class="stats-grid">
        ...
        <el-card shadow="hover" class="stat-card">
          ...
          >
            ...
            <template #extra>
              <el-tag type="success" size="small">+18.6%</el-tag>
              <span class="trend-period">较上月</span>
            </template>
          </el-statistic>
        </el-card>
...
import {
  User, ShoppingBag, UserFilled, Goods, Document, Coin, Download
} from '@element-plus/icons-vue'
import * as echarts from 'echarts'
import { getSuperAdminDashboardData } from '@/api/super-admin/dashboard'
...
const currentUser = ref({
  username: userStore.userInfo?.username || '超级管理员',
  role: 'super-admin'
})
...
const orderTypeDistribution = ref([])
...
const orderTypeChartRef = ref<HTMLElement | null>(null)
      ...
      series: [
        {
          ...
          type: 'pie',
          radius: '50%',
          data: orderTypeDistribution.value,
          emphasis: {
            itemStyle: {
              shadowBlur: 10,
              shadowOffsetX: 0,
...
const fetchDashboardData = async () => {
  try {
    ...
    if (response.code === 200) {
      ...
      orderTypeDistribution.value = data.orderTypeDistribution || []
